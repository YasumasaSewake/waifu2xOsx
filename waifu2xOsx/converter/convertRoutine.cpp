...
static bool convertWithModelsBasic(cv::Mat &inputPlane, cv::Mat &outputPlane,
		std::vector<std::unique_ptr<Model> > &models, std::string &resourcePath);
static bool convertWithModelsBlockSplit(cv::Mat &inputPlane,
                                        cv::Mat &outputPlane, std::vector<std::unique_ptr<Model> > &models, std::string &resourcePath);

bool convertWithModels(cv::Mat &inputPlane, cv::Mat &outputPlane,
                       std::vector<std::unique_ptr<Model> > &models, std::string &resourcePath, bool blockSplitting) {
...
//	requireSplitting = true;
	if (blockSplitting && requireSplitting) {
		return convertWithModelsBlockSplit(inputPlane, outputPlane, models, resourcePath);
	} else {
		...
		bool ret = convertWithModelsBasic(tempMat, outputPlane, models, resourcePath);
		if (ret == false) {
			return false;
		}
...
static bool convertWithModelsBasic(cv::Mat &inputPlane,
                                   cv::Mat &outputPlane,
	...
	try {
		...
		for (int index = 0; index < (int)models.size(); index++) {
			if (!models[index]->loadGLShader(resourcePath)) {
				std::exit(-1);
			}
		}
...
static bool convertWithModelsBlockSplit(cv::Mat &inputPlane,
	...
	unsigned int nModel = (unsigned int)models.size();
	...
	for (unsigned int r = 0; r < splitRows; r++) {
		...
		for (unsigned int c = 0; c < splitColumns; c++) {
			...
			std::cout << "process block (" << (c + 1) << "," << (r + 1) << ") ..."
					<< std::endl;
			if (!convertWithModelsBasic(processBlock, processBlockOutput, models, resourcePath)) {
